...
begin
    if not exists (
        select 1
        from pg_constraint
        where conname = 'diagrams_nodes_id_fk'
    ) then
        alter table public.diagrams
...
select 'system@arepos.local', '{"system":true,"seed":"liquibase"}'::jsonb
where not exists (
    select 1 from public.users where email = 'system@arepos.local'
);

insert into public.node_types (name, attrs, owner)
select
    'Directory',
    '{"system":true,"kind":"directory"}'::jsonb,
    u.id
from public.users u
