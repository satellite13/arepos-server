...
CREATE INDEX IF NOT EXISTS notations_name_trgm_idx ON public.notations USING gin (name gin_trgm_ops);
CREATE INDEX IF NOT EXISTS node_types_name_trgm_idx ON public.node_types USING gin (name gin_trgm_ops);
CREATE INDEX IF NOT EXISTS link_types_name_trgm_idx ON public.link_types USING gin (name gin_trgm_ops);
CREATE INDEX IF NOT EXISTS components_name_trgm_idx ON public.components USING gin (name gin_trgm_ops);
CREATE INDEX IF NOT EXISTS relations_name_trgm_idx ON public.relations USING gin (name gin_trgm_ops);
CREATE INDEX IF NOT EXISTS nodes_name_trgm_idx ON public.nodes USING gin (name gin_trgm_ops);
CREATE INDEX IF NOT EXISTS diagrams_name_trgm_idx ON public.diagrams USING gin (name gin_trgm_ops);
