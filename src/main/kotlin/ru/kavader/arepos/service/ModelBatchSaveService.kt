...
class ModelBatchSaveService(
        ...
        for (item in creates) {
            ...
            val saved = diagramsRepository.save(
                Diagrams(
                    ...
                    createdAt = now, updatedAt = now, attrs = remappedAttrs,
                    owner = owner, deleted = false, model = model,
                    notation = notation, node = node
                )
            )
    ...
    private fun updateDiagrams(
        ...
        for (upd in updates) {
            ...
            diagramsRepository.save(
                diagram.copy(
                    name = upd.name, version = upd.version, notation = notation,
                    node = node, attrs = remappedAttrs, updatedAt = now
                )
            )
        }
        return updates.size
    ...
    private fun findNodeOrThrow(
        id: UUID, message: String = "Node $id not found", status: HttpStatus = HttpStatus.NOT_FOUND
    ...
    private fun remapDiagramAttrs(
        attrs: String?, nodeIdMap: Map<String, UUID>, linkIdMap: Map<String, UUID>
    ): String? {
